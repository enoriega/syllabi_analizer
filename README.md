...
Basic usage (processes files in `data/` directory):
...
```bash
python extract_text.py --data-dir /path/to/documents
```

Specify a separate output directory:

```bash
python extract_text.py --data-dir data --output-dir output
```
...
3. Skips files that have already been processed (based on filename)
4. Uses an LLM to analyze each new syllabus and extract:
   - Course name and code
   - Term offered (semester and year)
   - Course description and objectives
   - Whether the course is AI-related
   - Justification for AI classification
5. Appends new results to existing ones and saves all parsed syllabi as JSON array

**Note**: The script automatically skips already-processed files, so you can safely re-run it to process new syllabi without incurring duplicate API costs.
