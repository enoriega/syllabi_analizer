...
The `parse_syllabi.py` script uses a language model to automatically extract structured information from syllabus text files. It supports parallel processing for faster throughput.
...
Basic usage with parallel processing (5 workers by default):
```bash
...
**Note**: More workers can process files faster, but may hit API rate limits. Adjust based on your API's rate limits and available resources.
...
5. Processes files in parallel using a worker pool (default: 5 workers)
6. Uses an LLM to analyze each new syllabus and extract:
   - Course name and code
   - Term offered (semester and year)
   - Course description and objectives
   - Whether the course is AI-related
   - Justification for AI classification
7. Shows progress updates every 100 files with current statistics
8. Appends new results to existing ones and saves all parsed syllabi as JSON array

**Features**:
- Parallel processing with configurable worker pool (default: 5 workers)
- Automatically skips already-processed files (no duplicate API costs)
- Progress updates every 100 files show current success/error counts and AI percentage
- Year filtering saves costs by processing only recent syllabi
- Resume capability: can interrupt and restart without losing progress
